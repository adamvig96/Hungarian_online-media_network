...
covid_data <- covid_data %>% separate(date, c("year", "month", "day"), "-")

class(covid_data$day)<-"numeric"
class(covid_data$month)<-"numeric"
class(covid_data$week)<-"numeric"
class(covid_data$year)<-"numeric"
