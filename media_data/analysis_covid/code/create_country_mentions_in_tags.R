...
for (name in usa_tags){
  covid_text <- covid_text %>% 
    mutate(tags = gsub(name, us[1,"country_hun"], tags))
}
...
for (name in uk_tags){
  covid_text <- covid_text %>% 
    mutate(tags = gsub(name, uk[1,"country_hun"], tags))
}
...
# use TAGS to identify country related articles
covidcorpus <- corpus(covid_text$tags)
...
names(country_by_week_m)<-c("week_page", "country_hun", "mentions_tags")
